...
기본 저장소는 `markdown files + SQLite metadata` 입니다. 현재 SQLite 운영은 단순 기본값이 아니라 아래 정책을 전제로 합니다.

- `WAL` 모드 사용
- sync 시작 전 DB lease를 획득해서 `bootstrap` / `incremental` writer가 겹치지 않게 차단
- sync 종료 후 `PRAGMA optimize`, `PRAGMA wal_checkpoint(PASSIVE)` 실행
- admin 화면이나 내부 스케줄러에서 동시에 sync를 눌러도, 이미 다른 sync가 돌고 있으면 새 작업은 실패로 정리되고 UI에 원인이 표시됨

이 구조는 단일 인스턴스 또는 소규모 운영에는 충분하지만, 아래 경우에는 PostgreSQL 전환을 권장합니다.

- 여러 앱 프로세스를 동시에 띄우는 경우
- admin 작업과 sync가 자주 겹치는 경우
- 동시 사용자 수가 늘어나는 경우
- 검색, 작업 큐, 이력 관리가 더 복잡해지는 경우

DB 이전 절차는 [db_migration.md](./db_migration.md) 에 정리했습니다.
...
SQLite metadata schema 변경이 있으면 항상 아래를 먼저 실행합니다.
...
- [docs/plans/2026-04-07-obsidian-compat-design.md](./docs/plans/2026-04-07-obsidian-compat-design.md)
- [docs/plans/2026-04-07-obsidian-compat.md](./docs/plans/2026-04-07-obsidian-compat.md)

## 추가 문서

- [db_migration.md](./db_migration.md)
- [alembic_upgrade_head.md](./alembic_upgrade_head.md)
